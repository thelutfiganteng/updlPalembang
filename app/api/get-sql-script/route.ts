...
export async function GET() {
  try {
    // In a real production environment, you would read from the actual file
    // For this example, we'll just return the SQL script directly
    const sqlScript = `
-- Create users table
...
CREATE INDEX IF NOT EXISTS idx_borrow_records_item_id ON borrow_records(item_id);
    ...
    console.error("Error serving SQL script:", error)
    return new NextResponse("Error serving SQL script", { status: 500 })
  }
}
